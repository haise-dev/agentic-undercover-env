...
test("API_BASE_URL defaults to http://localhost:8000 when env var not set", async () => {
  ...
  const { API_BASE_URL, WS_BASE_URL } = await import(`../lib/constants.ts?t=1`);
...
test("WS_BASE_URL replaces http with ws correctly", async () => {
  ...
  const { API_BASE_URL, WS_BASE_URL } = await import(`../lib/constants.ts?t=2`);
...
test("WS_BASE_URL replaces https with wss correctly", async () => {
  ...
  const { API_BASE_URL, WS_BASE_URL } = await import(`../lib/constants.ts?t=3`);
